"""
Pihu ΓÇö Deterministic Routing Engine
...
class RouteResult:
    ...
    tool_announcement: str  # What to tell user ("Searching webΓÇª")
...
class Router:
    ...
    ΓöîΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓö¼ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÉ
    Γöé Condition          Γöé Action                  Γöé
    Γö£ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓö╝ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöñ
    Γöé Time-sensitive     Γöé Web Search FIRST        Γöé
    Γöé Complex reasoning  Γöé Cloud LLM               Γöé
    Γöé Normal chat        Γöé Local LLM               Γöé
    Γöé UI request         Γöé MCP Tool                Γöé
    Γöé "Look / Screen"   Γöé Vision pipeline          Γöé
    Γöé System command     Γöé Local execution          Γöé
    ΓööΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓö┤ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÿ
    
    Anti-Hallucination: if confidence < threshold ΓåÆ force tool usage.
    """
    ...
    def __init__(
        ...
        log.info("Router initialized | Groq=%s, OpenClaw=%s",
                 "Γ£à" if groq_llm and groq_llm.is_available else "Γ¥î",
                 "Γ£à" if openclaw else "Γ¥î")
    ...
    def route(self, intent: Intent) -> RouteResult:
        ...
        # 4. Anti-hallucination: low confidence ΓåÆ force tool
        if intent.confidence < self.confidence_threshold and intent.type == "chat":
            log.warning(
                "Low confidence (%.0f%%) on chat ΓåÆ forcing web search",
    ...
    def _build_context_prompt(self, intent: Intent) -> str:
        ...
        if context:
            ...
            if len(ctx_str) > 500 and hasattr(self, "rag_engine") and self.rag_engine.is_available:
                log.info("≡ƒº⌐ Triggering RAG Cognitive Filter...")
    ...
    def _route_chat(self, intent: Intent) -> RouteResult:
        """Chat routing: Local (Primary) ΓåÆ Cloud (On-Demand Only)."""
        ...
        # ΓöÇΓöÇΓöÇ ON-DEMAND OR PRIMARY CLOUD PATH (NVIDIA NIM) ΓöÇΓöÇΓöÇ
        if use_cloud or not NVIDIA_NIM_ON_DEMAND:
            if self.cloud_llm and self.cloud_llm.is_available and NVIDIA_NIM_API_KEY:
                log.info("Γÿü∩╕Å CLOUD PATH: Using NVIDIA NIM as primary brain")
        ...
        # ΓöÇΓöÇΓöÇ LOCAL PRIMARY PATH (Ollama / TurboQuant) ΓöÇΓöÇΓöÇ
        log.info("≡ƒÅá LOCAL PATH: Using local LLM with TurboQuant")
    ...
    def _route_realtime_query(self, intent: Intent) -> RouteResult:
        """Time-sensitive ΓåÆ Web Search FIRST, then LLM summarization."""
        announcement = "≡ƒöì Search kar rahi hoon..."
...
Answer in Hinglish naturally. Be factual ΓÇö use only the search results."""
    ...
    def _route_deep_reasoning(self, intent: Intent) -> RouteResult:
        """Complex reasoning ΓåÆ Cloud LLM with local fallback."""
        announcement = "≡ƒºá Deep analysis kar rahi hoon..."
    ...
    def _route_vision(self, intent: Intent) -> RouteResult:
        """Vision ΓåÆ Screen capture + Vision model."""
        announcement = "≡ƒæü∩╕Å Screen dekh rahi hoon..."
    ...
    def _route_ui_generation(self, intent: Intent) -> RouteResult:
        """UI generation ΓåÆ MCP dispatcher."""
        announcement = "≡ƒÄ¿ UI generate kar rahi hoon..."
...
Provide the HTML/CSS/JS code."""
    ...
    def _route_system_command(self, intent: Intent) -> RouteResult:
        """System command ΓåÆ TaskWeaver Planner ΓåÆ OpenInterpreter Execution."""
        announcement = "≡ƒºá Planning and executing..."
        ...
        def synergistic_execution_stream():
            ...
            if is_multi_step and hasattr(self, "planner_engine") and self.planner_engine.is_available:
                log.info("≡ƒôÉ Triggering TaskWeaver Planner for complex request")
            ...
            if hasattr(self, "interpreter_engine") and self.interpreter_engine.is_available:
                log.info("≡ƒöº Routing to OpenInterpreter Sandbox")
        ...
        if self.openclaw:
            log.info("≡ƒ¢í∩╕Å Routing to OpenClaw Secured Orchestrator")
            return RouteResult(
                pipeline="system",
        ...
        if self.automation:
            log.info("ΓÜÖ∩╕Å Routing to Direct Automation Tool")
            return RouteResult(
                pipeline="system",
